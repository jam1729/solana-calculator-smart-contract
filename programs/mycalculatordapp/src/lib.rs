...
pub mod mycalculatordapp {
    ...
    pub fn subtract(ctx: Context<Subtraction>, num1: i64, num2: i64) -> ProgramResult {
        let caluclator = &mut ctx.accounts.calculator;
        caluclator.result = num1 - num2;
        Ok(())
    }
    ...
    pub fn divide(ctx: Context<Division>, num1: i64, num2: i64) -> ProgramResult {
        let calculator = &mut ctx.accounts.calculator;
        if (num2 == 0) {
            msg!("num2 is 0. Division would lead to division by zero error!");
        }
        calculator.result = num1 / num2;
        calculator.reminder = num1 - num2 * calculator.result;
        Ok(())
    }
}
